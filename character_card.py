...
from network import fetch_image, get_session
# ------------------ GLOBAL CACHE ------------------
STAT_ICON_CACHE = {}
SMALL_ICON_CACHE = {}
IMAGE_CACHE = {}
...
def load_icon(path, size):
    key = (path, size)
    if key not in STAT_ICON_CACHE:
        try:
            STAT_ICON_CACHE[key] = Image.open(path).convert("RGBA").resize(size)
        except:
            STAT_ICON_CACHE[key] = None
    return STAT_ICON_CACHE[key]
...
async def characters_card(uid, char_id, telegram_id):
    ...
    # ------------------ PARALLEL FETCH ------------------
    bg_task = asyncio.create_task(fetch_image(session, get_namecard_urls(avatar_icon)[0]))
    splash_task = asyncio.create_task(fetch_image(session, get_splash_url(avatar_icon)))
    weapon_task = asyncio.create_task(fetch_image(session, f"https://enka.network/ui/{stats['weapon'].get('icon')}"))

    graph_task = loop.run_in_executor(None, lambda: get_complete_radar_module(stats, char_id, (400, 400)))

    bg_img, splash_img, weapon_img, graph_img = await asyncio.gather(
        bg_task, splash_task, weapon_task, graph_task
    )
    ...
    bg = ImageOps.fit(bg_img, target_size).convert("RGBA")
    ...
    # ------------------ TEXT ------------------
    draw_text_with_shadow(draw, char_name, (50, 50), font_path, 36, anchor="lm")
    draw_text_with_shadow(draw, me['nickname'], (250, 50), font_path, 26, anchor="lm")

    # ------------------ WEAPON ------------------
    if weapon_img:
        w_pos = (900, 20)
        ui_layer.paste(ImageOps.contain(weapon_img, (140, 140)), w_pos)

        w_info = stats['weapon']
        draw_text_with_shadow(draw, get_weapon_name(w_info), (w_pos[0] + 170, w_pos[1] + 30), font_path, 32, anchor="lm")

    # ------------------ WEAPON STATS ------------------
    stat_x_start = 1070
    stat_y = 120

    for i, s in enumerate(stats['weapon'].get("stats", [])):
        x = stat_x_start + (i * 125)

        draw.rounded_rectangle([x, stat_y, x + 115, stat_y + 40], radius=5, fill=(255, 255, 255, 100))

        icon = load_icon(W_STAT_ICONS.get(s['prop'], "asstests/icons/atk.png"), (22, 22))
        if icon:
            ui_layer.paste(icon, (x + 5, stat_y + 10), icon)

        val = f"{s['val']}"
        if "PERCENT" in s['prop'] or "CRITICAL" in s['prop']:
            val += "%"

        draw.text((x + 40, stat_y + 20), val, font=font_small, fill=(255, 255, 255), anchor="lm")
    ...
    for i, (label, key, fmt, icon_path) in enumerate(stat_config):
        ...
        icon = load_icon(icon_path, (35, 35))
        if icon:
            ui_layer.paste(icon, (850, y), icon)

        draw_text_with_shadow(draw, label, (900, y + 18), font_path, 24, anchor="lm")
        draw_text_with_shadow(draw, fmt.format(stats.get(key, 0)), (1400, y + 18), font_path, 26, anchor="rm")

    # ------------------ ARTIFACTS (CORRECT) ------------------
    await draw_horizontal_artifacts(
        session, ui_layer, char_info, 150, 650,
    ...
    if graph_img:
        radar_bg = load_icon("asstests/icons/radar_bg.png", (530, 525))
        if radar_bg:
            ui_layer.paste(radar_bg, (graph_position[0] - 70, graph_position[1] - 52), radar_bg)

        ui_layer.paste(graph_img, graph_position, graph_img)
